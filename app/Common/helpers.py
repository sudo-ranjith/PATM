...
def sent_mail(mail_dict):
    ...
    try:
        ...
        for _key in key_name:
            ...
            message = f"""\
            வணக்கம் {user_name},
                நீங்கள் {feeding_report_month} நாட்களில் {fire_call_count} தீயணைப்பு மற்றும் {purchase_module_count} மீட்புப் பணியில் சிறப்பாக பணி ஆற்றியமைக்கு வாழ்த்துகள்.
...
def upload_image(file):
    """
    store the image file in upload_img dir and return the file name
    """
    func_resp = {}
    try:
        ...
        func_resp['message'] = "File uploaded successfully"
        func_resp['status'] = "success"
        func_resp['data'] = file_name
    except Exception as e:
        func_resp['message'] = traceback.format_exc()
        func_resp['status'] = "failed"
    finally:
